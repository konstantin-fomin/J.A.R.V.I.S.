...
- **Коммит ≠ деплой.** Код запекается в образ через `COPY` (не bind-mount — монтируются
  только `.db`), поэтому `git commit`/`merge` НЕ обновляет работающий контейнер. После
  мерджа фикса, который надо проверить «на проде», обязателен шаг
  `docker compose up -d --build` (+ `docker compose logs --tail` на старте). Не считай
  «закоммичено» индикатором «работает на проде»: проверяй живой контейнер
  (`docker compose exec bot grep …` по новому коду / реальный запрос к боту или веб-`/chat`).
