...
- [ ] Свободный текст → structured intent через Gemini (create_task / create_reminder / query_tasks и т.д.)
- [ ] **Голосовые сообщения** → транскрипция + intent в одном вызове Gemini — см. техническую заметку в разделе 5
- [ ] Подтверждение для опасных действий (delete, cancel, изменения календаря)
- [ ] Confidence threshold — переспрос при низкой уверенности интента
