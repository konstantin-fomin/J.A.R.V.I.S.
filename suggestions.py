...
# Автор неудавшейся команды/конфабуляции на chat-пути (см. intents.REFUSAL_AUTHOR).
# Такой обмен — не мысль пользователя, а попытка действия, которую бот не
# распознал: если журналировать её как обычную запись, повторные попытки одной
# и той же команды образуют кластер и §13 предложит задачу из текста отказа.
_REFUSAL_AUTHOR = "отказ"

_NON_CONTENT_AUTHORS = (_BOT_AUTHOR, _REFUSAL_AUTHOR)


def user_authored_text(text: str) -> str:
    """Оставляет в journal-чанке только записи, написанные пользователем.

    Реплики бота (`[бот]`) и отказы/конфабуляции (`[отказ]`) — не источник для
    проактивных подсказок: иначе бот предлагает задачи по собственным же ответам
    (эхо-петля, §13) или по неудавшимся попыткам команд. Строки-продолжения
    многострочной записи наследуют автора своей записи.
    ...
    for ln in lines:
        ...
        if m is not None:
            keep = m.group(1).strip() not in _NON_CONTENT_AUTHORS
        elif not ln.strip():
